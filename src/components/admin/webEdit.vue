  ...
  <div>
    <div>
      ...
      <el-form :model="webInfo" :rules="rules" ref="ruleForm" label-width="100px"
        ...
        <el-form-item label="页脚" prop="webFootor">
          <el-input v-model="webInfo.webFootor"></el-input>
        </el-form-item>

        <el-form-item label="背景" prop="cover">
          <div style="display: flex">
            <el-input v-model="webInfo.cover"></el-input>
            <el-image lazy class="table-td-thumb"
                      style="margin-left: 10px"
                      :preview-src-list="[webInfo.cover]"
                      :src="webInfo.cover"
                      fit="cover"></el-image>
          </div>
        ...
        <el-form-item label="头像" prop="webHead">
          <div style="display: flex">
            <el-input v-model="webInfo.webHead"></el-input>
            <el-image lazy class="table-td-thumb"
                      style="margin-left: 10px"
                      :preview-src-list="[webInfo.webHead]"
                      :src="webInfo.webHead"
                      fit="cover"></el-image>
          </div>
    ...
    data() {
      return {
        ...
        rules: {
          ...
          webNotices: [
            {required: true, message: '请设置网站提示信息', trigger: 'blur'}
          ],
          webHead: [
            {required: true, message: '请上传默认头像', trigger: 'blur'}
          ]
        }
      }
    },
    ...
    methods: {
      ...
      updateWebInfo(value) {
        this.$confirm('确认保存？', '提示', {
          ...
          this.$http.post(this.$constant.baseURL + "/webInfo/update", value, true)
            .then((res) => {
              this.getWebInfo();
              this.$message({
                message: "保存成功！",
