    ...
    <loader :loading="loading">
      ...
      <template slot="body">
        ...
        <div class="signature-wall myCenter my-animation-hideToShow">
          <h1 class="playful">
            <span v-for="(a, index) in ['你','好','世','界']" :key="index">{{a}}</span>
          </h1>
        ...
        <div class="page-container-wrap">
          <div class="page-container">
            <div class="aside-content" v-if="showAside">
              <myAside @selectArticle="selectArticle"></myAside>
            </div>
            <div class="recent-posts">
              <div class="announcement background-opacity">
                <i class="fa fa-volume-up" aria-hidden="true"></i>
                <div>
                  {{$store.state.webInfo.webNotices}}
...
<!--                  </div>-->
              ...
              <div v-show="indexType === 1">
                <div v-for="(sort, index) in sortInfo" :key="index">
                  <div v-if="!$common.isEmpty(sortArticles[sort.uid])">
                    <div class="sort-article-first">
                      ...
                      <div class="article-more" @click="$router.push({path: '/sort', query: {sortId: sort.uid}})">
                        <svg viewBox="0 0 1024 1024" width="20" height="20"
                             style="vertical-align: -2px;margin-bottom: -2px">
                          <path
                    ...
                    <sortArticle :articleList="sortArticles[sort.uid]"></sortArticle>
                  </div>
                </div>
              </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      // setTimeout(() => {
      //   this.push = this.$common.pushNotification(this.$store.state.webInfo.webNotices, false);
    ...
    methods: {
      ...
      async getArticles() {
        await this.$http.post(this.$constant.baseURL + "/blog/selectAll", this.pagination)
          .then((res) => {
